...
test("platform mode nav keeps a stable visual center across country views", async () => {
  ...
  const mobileModeNavBlock = styles.match(/@media \(max-width: 640px\)\s*\{[\s\S]*?\.modeNavAnchor\s*\{(?<body>[\s\S]*?)\}/)?.groups?.body ?? "";
  ...
  assert.match(styles, /\.modeNavAnchorWorkView\s*\{[\s\S]*max-width: min\(360px, calc\(100% - 260px\)\);/);
  ...
  assert.match(styles, /@media \(max-width: 640px\)\s*\{[\s\S]*\.modeNavAnchor\s*\{[\s\S]*position: fixed;[\s\S]*top: calc\(16px \+ env\(safe-area-inset-top, 0px\)\);[\s\S]*bottom: auto;[\s\S]*z-index: 7;/);
  assert.match(styles, /@media \(max-width: 640px\)\s*\{[\s\S]*\.shellNoMapChrome \.modeNavAnchor\s*\{[\s\S]*top: calc\(16px \+ env\(safe-area-inset-top, 0px\)\);[\s\S]*bottom: auto;[\s\S]*left: 50%;[\s\S]*max-width: 112px;[\s\S]*transform: translateX\(-50%\);/);
  assert.doesNotMatch(mobileModeNavBlock, /bottom: 18px;/);
  assert.match(styles, /@media \(max-width: 640px\)\s*\{[\s\S]*\.modeNavAnchorWorkView\s*\{[\s\S]*max-width: 112px;/);
