...
test("buildCoverageReport summarizes source readiness and case evidence coverage", () => {
  ...
  assert.equal(report.countries.AR.caseFiles, 558);
  assert.equal(report.countries.AR.caseFilesWithReceipts, 558);
  assert.equal(report.countries.AR.caseFilesWithCoordinates, 461);
  assert.equal(report.countries.PE.mvpSources >= 1, true);
  assert.equal(report.countries.PE.sourcesWithSnapshots, 3);
